"""真实模型供应商策略前端入口：模型广场「真实模型」tab 卡片上的供应商策略弹框。
...
def test_api_exposes_real_model_routing_narrow_write():
    ...
    # 列表接口回传的 real 行供应商策略列要在类型里可读，否则弹框拿不到回填数据
...
def test_marketplace_card_has_routing_entry_next_to_edit():
    ...
    # 按算法挑候选）不是同一件事——这里配的是允许哪些供应商承接，故用「供应商策略」。
    assert ">供应商策略</Button>" in actions
...
def test_routing_dialog_does_not_repeat_carrier_info_from_card():
    """卡片上已有「模型在哪些供应商」的标签；弹框里再列命中供应商是重复信息，已砍掉。"""
    source = DIALOG.read_text(encoding="utf-8")

    # 弹框顶部的「承载供应商 N」徽章与卡片 providerTags 重复，已删除
    assert "承载供应商" not in source
    # 方案列表里的「N 个命中供应商」与卡片重复，已删除
    assert "个命中供应商" not in source
...
def test_shared_field_widgets_exported_from_model_routing():
    ...
    # 供应商策略弹框复用自定义模型页的字段/多选控件，避免两处供应商标签选择行为漂移
