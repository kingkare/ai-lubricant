...
# 进程内一次性门闩：节点 dial-back URL 配成回环时只警告一次（每个 onboard 都会走这条路，
# 但日志风暴无用），让"远程机器节点连不上"这类配置坑能被服务端日志直接定位。
_LOOPBACK_DIALBACK_WARNED = False
...
admin_router = APIRouter(prefix="/api/v1/admin", tags=["user-platform-nodes-admin"])
# Team self-service node surface.
team_router = APIRouter(prefix="/api/v1/teams", tags=["user-platform-nodes-team"])
...
class CreateExecutionNodeReq(BaseModel):
    ...
    node_name: str | None = None
    image: str | None = None
    # 该节点绑定的出口代理池条目 id（空 = 直连）。落进节点记录 + install bootstrap，
    # 节点从 GitHub 下载安装/升级二进制走它；与 admin onboard 同口径。
    proxy_config_id: str = ""
...
_REASON_STATUS = {
    ...
    # 数据侧 select/解析类错误，与控制面 RPC 同名代码对齐（412/400）。
...
def _node_server_url(request: Request) -> str:
    ...
    if configured:
        # 远程节点用这个地址拨回控制面。配成 127.0.0.1/localhost 时，只有同机
        # 节点能连（别的机器上 127.0.0.1 指向它自己）——远程节点会永远停在
        # 「等待节点上线」。有显式配置就说明管理员在意这个值，回环几乎必是坑。
        global _LOOPBACK_DIALBACK_WARNED
        host = (urlparse(configured).hostname or "").lower()
        if host in ("127.0.0.1", "localhost", "::1", "") and not _LOOPBACK_DIALBACK_WARNED:
            _LOOPBACK_DIALBACK_WARNED = True
            logger.warning(
                "[nodes] AGENT_COMPOSE_NODE_SERVER_PUBLIC_URL 配成了回环地址 {}，"
                "远程机器上的节点将拨到自己机器而永远连不上；如需远程节点接入，"
                "请改为本机局域网/公网可达地址", configured,
            )
        return configured
...
@admin_router.post("/nodes/{node_id}/host-tools/{tool}/install")
async def admin_install_node_host_tool(
    node_id: str, tool: str, request: Request, _: User = Depends(_require_admin)
) -> dict:
    """在该节点上安装宿主级运行依赖（当前仅 nodejs：Node.js + npm）。

    环境面板/审批弹窗的「安装」按钮走这里。下载在节点上经其出口代理执行，
    控制面守卫与编辑器安装同口径（在线即可，不要求已审批——审批门禁本身
    就卡在缺 Node.js 上）。
    """
    body: dict = {}
    try:
        ...
        body = {}
    return await _guard(
        nodes_service.install_host_tool(
            node_id,
            tool,
...
@team_router.post("/nodes/{node_id}/approve")
async def team_approve_node(node_id: str, user: User = Depends(get_current_user)) -> dict:
    """成员侧审批：对用户有权管理的节点放行审批。

    权限口径与编辑器/Node.js 安装一致（``user_can_use_node`` 派生管理权：直接绑定
    或经所属已绑定管理节点派生）。这让拥有管理节点权限的成员在用户侧完成执行
    节点的「添加 → 自动拉起 → 审批」闭环，不必回管理端。
    """
    if await nodes_service.user_can_use_node(str(user.id), node_id) is None:
        raise HTTPException(status_code=403, detail="无权管理该节点")
    return await _guard(nodes_service.approve_node(node_id))
...
@team_router.post("/nodes/{node_id}/host-tools/{tool}/install")
async def team_install_node_host_tool(
    node_id: str, tool: str, user: User = Depends(get_current_user)
) -> dict:
    """成员侧同一安装能力：节点装 Node.js/npm（权限口径与编辑器安装一致）。"""
    if tool != "nodejs":
        raise HTTPException(status_code=400, detail="不支持该宿主工具")
    if await nodes_service.user_can_use_node(str(user.id), node_id) is None:
        raise HTTPException(status_code=403, detail="无权管理该节点")
    return await _guard(nodes_service.install_host_tool(node_id, tool))
