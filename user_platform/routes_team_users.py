...
These endpoints mirror the upstream team handler contract so the ported
...
router = APIRouter(prefix="/api/v1/teams", tags=["user-platform-team-users"])
...
async def dashboard(
    ...
    The frontend expects the upstream ``TeamDashboardResp`` shape. Keep this
