...
def _normalize_active_names(raw: Any) -> list[str] | None:
    """Normalize an env-activation subset (active_skills/active_plugins).

    Non-string/blank entries are dropped, order is kept, duplicates removed.
    ``None``/empty input → ``None`` (means "activate everything the
    environment has" downstream — an empty subset is not expressible).
    """
...
async def _validate_models_for_key(
    ...
    *,
    ...
    收口任务/编辑器写入边界：模型必须落在该 Key 当前可选集合内。可选集合统一
    取 ``ModelClientPool.get_models_response(api_key=...)`` —— 这正是 /chat/models
    给前端 keyModels 列表用的同一个函数（含自定义模型组、按白/黑名单与可供给性
    过滤）。校验与 UI 同源，组名与真实模型一视同仁，不会出现「UI 列得出、校验
    拒绝」的漂移（曾因组名不在 available_model_ids() 里被拒，导致不限制任务首
    次切模型报「目标模型不在任务可用范围内」）。返回过滤后的合法模型列表与激
    活模型（激活模型若非法回退到首个合法模型，全空时为 None）。
    """
    ...
    # 解析明文 Key：草稿（无父 Key）传 None，get_models_response 不做 Key 级过滤，
    # 仍按可供给性 + 内部 id 过滤。有 Key 时从 DB 取明文（仅校验用，不落库）。
    ...
    legal = [m for m in cleaned if m in selectable]
    if not legal:
...
async def _task_detail(task: Task) -> dict:
    ...
    # mcp_config 改从 principal grants 派生（名字/transport 列表，无密钥）；
    # 退役的 mcp_config 列不再作为 DTO 来源。
    detail["mcp_config"] = await _task_mcp_grant_summary(task)
...
async def _task_mcp_grant_summary(task: Task) -> list[dict]:
    """Detail-DTO MCP list derived from the task principal's grants.

    名称 + 生效 transport（stdio=节点本地 spawn；其余=经服务端网关代理的
    sse），无 url/token/env 等任何密钥字段——前端选择器据此渲染与 diff，
    真正的 wire spec 在派发时按 grants 现生成。
    """
    principal_id = getattr(task, "mcp_user_id", None)
    if not principal_id:
        return []
    import mcp_plugin_store
    from .resource_reference_service import mcp_service_is_stdio

    try:
        service_ids = await mcp_plugin_store.list_services_for_mcp_user(int(principal_id))
    except Exception:
        logger.exception("[user-platform] task {} grant summary read failed", task.id)
        return []
    out: list[dict] = []
    for service_id in service_ids:
        try:
            service = await mcp_plugin_store.get_service(int(service_id))
        except Exception:
            continue
        ...
        out.append({
            ...
            "transport": "stdio" if mcp_service_is_stdio(service) else "sse",
        })
    return out
...
async def _task_principal_view(task: Task) -> dict | None:
    ...
    lifecycle and the detail-page MCP permission editor, not through this DTO.

    Phase 3：grants 全景展示——service 行（授权服务集）+ param 行（实例绑
    定，多值语义）。不再从 mcp_config 列派生任何内容。
    """
    ...
    if not principal or principal.get("usage_type") != "task":
        ...
        return None
    try:
        grants = await mcp_plugin_store.list_principal_grants(task.mcp_user_id)
    except Exception:
        logger.exception("[user-platform] task {} principal view failed", task.id)
        grants = []
    services: list[int] = []
    params: list[dict] = []
    for grant in grants:
        key = str(grant.get("grant_key") or "")
        value = str(grant.get("grant_value") or "")
        if not key or not value:
            continue
        if key == mcp_plugin_store.GRANT_SERVICE_KEY:
            try:
                services.append(int(value))
            except (TypeError, ValueError):
                continue
        else:
            params.append({"param_key": key, "param_value": value})
    return {
...
class TaskService:
    ...
    async def create_task(self, user_id: str, req: dict, *, defer_dispatch: bool = False) -> dict:
        ...
        # ``mcp_config`` 不再映射进 _session_mcps：派发层改从 task principal 的
        # grants 推导（见 _principal_mcp_specs），请求负载翻译成 grants 写入。
        req = dict(req)
        ...
        task = await Task.create(
            ...
            config_snapshot={
                ...
                # 环境自带资源的激活子集（名字列表）。空/缺失 = 全激活；非空 =
                # 只激活这些。写入边界见 _build_session 的 shared/system 分支。
                "active_skills": _normalize_active_names(req.get("active_skills")),
                "active_plugins": _normalize_active_names(req.get("active_plugins")),
            },
            # mcp_config 列退役（Phase 3）：不再持久化 wire specs，任务 MCP 的
            # 真相源是 task principal 的 mcp_grants。ORM 字段留到 Phase 4 清理。
        ...
        # 创建即绑定 task principal（对齐 Agent 创建即建 principal 的管理模型）：
        # 详情页的 MCP 权限编辑器在任务尚未派发时就有 principal 可配，不必等首次
        # 派发。Phase 3 起 _ensure 只保证「存在 + 启用」，grants 由下一行的请求负
        # 载翻译与权限编辑器写入，派发/恢复路径不会重算覆盖。
        # best-effort：失败不阻断创建，派发路径会再尝试补建。
        await self._ensure_task_mcp_principal(task)
        # mcp_config 请求负载（{service_id}/{resource_id} 绑定或已解析 spec）
        # → principal grants（service 行）写入。grants 是单一真相源，task.mcp_config
        # 列已退役不再写。best-effort：principal 未建成时跳过，派发路径会再尝试。
        await self._sync_task_mcp_service_grants(task, req.get("mcp_config"))
        ...
        if not client.enabled:
            logger.info(
                "[user-platform] task {} created (storage only; node server disabled)",
                task.id,
            )
    ...
    async def _schedule_created_dispatch(
        ...
        if content:
            ...
            try:
                ...
                logger.exception(
                    "[user-platform] task {} initial input persist failed", task.id
                )
        ...
        def _finish(future: asyncio.Task) -> None:
            ...
            try:
                ...
                logger.opt(exception=exc).error(
                    "[user-platform] task {} deferred dispatch crashed", task.id
                )
    ...
    async def _dispatch_created_task(
        ...
        *,
        ...
        try:
            ...
            # 派发前确保 principal「存在 + 启用」；创建时建失败的话在此补建，并
            # 重放一次 grants（否则 _principal_mcp_specs 读到空，会话静默丢工具）。
            await self._ensure_task_mcp_principal(task)
            if req.get("mcp_config"):
                await self._sync_task_mcp_service_grants(task, req.get("mcp_config"))
            # session mcps 改从 principal grants 推导（kind-aware 网关 spec / stdio
            # command 行），不再抄 task.mcp_config 的全量 wire specs（含上游密钥）。
            # req 里已有的 _session_mcps（内部调用方，如 webhook review）作为 base，
            # 同名时 grants 派生的安全 spec 胜出；仅 base 独有的条目（会话内工具）
            # 原样保留。
            req["_session_mcps"] = _merge_mcp_config(
                req.get("_session_mcps") if isinstance(req.get("_session_mcps"), list) else None,
                await self._principal_mcp_specs(task),
            )
            ...
            if content := (task.content or "").strip():
                ...
                try:
                    ...
                    logger.exception(
                        "[user-platform] task {} initial input push failed", task.id
                    )
            ...
            logger.info("[user-platform] task {} dispatch deferred: {}", task.id, exc)
            ...
            logger.info(
                "[user-platform] task {} dispatch {} (code={}): {}",
            ...
            logger.exception("[user-platform] task {} orchestration failed", task.id)
            await self._disable_task_mcp_principal(task)
    ...
    async def _cancel_pending_first_turn(task: Task) -> None:
        ...
        try:
            ...
            logger.exception("[user-platform] task {} cancel pending first turn failed", task.id)
    ...
    async def _refresh_api_key_snapshot() -> None:
        ...
        try:
            ...
            logger.warning("[user-platform] API Key 快照刷新失败: {}", exc)
    ...
    async def _rewrite_repo_for_git_proxy(self, req: dict, task: Task) -> None:
        ...
        try:
            ...
            logger.exception("[user-platform] task {} submodule allowlist build failed", task.id)
    ...
    async def _persist_submodule_allowlist(self, task: Task, repo_url: str, branch: str) -> None:
        ...
        try:
            ...
            logger.info(
                "[user-platform] task {} .gitmodules fetch failed (submodule proxy disabled): {}",
                task.id, exc,
            )
            return
        ...
        try:
            ...
            logger.exception(
                "[user-platform] task {} submodule allowlist persist failed", task.id
            )
    ...
    def _build_session(self, req: dict, task: Task) -> dict:
        ...
        if env_mode and env_mode != "isolated":
            ...
            # Shared AND system tiers: the task activates a SUBSET of the
            # environment's installed resources. An empty/absent list means
            # "everything the environment has" (the node enumerates it). These
            # names never install or remove anything — that is the
            # environment's job — they just pick what this run turns on, so a
            # task deactivating a skill cannot uninstall it for other tasks on
            # the same environment. System tier works the same way: the node
            # narrows the operator's HOME skills with the same explicit-list
            # rule (activeSkillNames does not branch on env_mode), it only
            # refuses to WRITE into that HOME.
            if env_mode in ("shared", "system"):
    ...
    async def _record_runtime_error(task: Task, reason: str) -> None:
        ...
        try:
            ...
            logger.exception(
                "[user-platform] task {} runtime error persist failed", task.id
            )
    ...
    async def _clear_runtime_stage(task: Task) -> None:
        ...
        try:
            ...
            logger.exception(
                "[user-platform] task {} runtime stage clear failed", task.id
            )
        ...
        async with lock:
            ...
            if existing:
                ...
                try:
                    ...
                    stale_handle = True
                    logger.info(
                        "[user-platform] task {} handle {} stale (binding gone); re-dispatching",
                        current.id, existing,
                    )
                else:
            ...
            req: dict[str, Any] = {
                ...
                # mcp_config 列已退役：恢复派发的 mcps 一并从 principal grants
                # 推导（与首次派发同一真相源），legacy 行的 grants 在创建时已同步。
                "_session_mcps": await self._principal_mcp_specs(current),
            ...
            try:
                ...
                retryable = exc.code == Code.UNAVAILABLE
                logger.info(
                    "[user-platform] task {} delayed dispatch {} (code={}): {}",
                ...
                logger.exception(
                    "[user-platform] task {} delayed dispatch failed", current.id
                )
                await self._disable_task_mcp_principal(current)
    ...
    async def _attach_issue_workflow_mcp(self, task: Task, req: dict, session: dict) -> None:
        ...
        try:
            ...
            # task identity token：网关 identity 分支按 target_type='task' 反查
            # Task.mcp_user_id → principal 判权，替代拿 "agent" 类型装 task id
            # 的旧 hack。注意：sse_gateway 的 issue-workflow 特判目前只放行
            # ("agent","node")，task 类型需网关侧同步放行（另一条线，串行做）。
            _row, token = await builtin_tool_store.issue_token(
                "task", str(task.id), display_token=False
            )
        except Exception:
            logger.exception("[user-platform] issue-workflow token issuance failed")
            return
        # 节点自拼形态：url 只发相对路径，节点拿服务端 hello 通告的网关 origin
        # 自己拼绝对地址（task_service 不再依赖 gateway_public_url 配置）。
        # 旧注释里「settings.server_url 笔误」的问题随之消亡——base 参数整体不传。
        entry = {
            "name": "issue-workflow",
            "type": "sse",
            "url": f"/mcp/issue-workflow/sse?token={token}",
        }
    ...
    async def _ensure_task_mcp_principal(self, task: Task) -> None:
        """Ensure the task-scoped MCP principal exists and is enabled.

        Phase 3 收敛：授权（service 行 / param 行）的真相源是 mcp_grants，由
        create/update 的 ``_sync_task_mcp_service_grants`` 和详情页 MCP 权限编辑
        器写入；本函数不再从 task.mcp_config / extra.builtin_tool_params 重算
        同步——那会在每次派发/恢复时覆盖编辑器改过的 grants（同步死写）。
        ...
        try:
            principal_id = await mcp_plugin_store.create_task_mcp_principal(
                str(task.user_id), f"task-{str(task.id)[:8]}",
                description=f"task principal for {task.id}",
            )
        except Exception:
            logger.exception(
                "[user-platform] task {} principal create failed", task.id
            )
            return
        ...
        try:
            ...
            logger.exception(
                "[user-platform] task {} mcp_user_id persist failed", task.id
            )

    async def _principal_mcp_specs(self, task: Task) -> list[dict]:
        """Derive the node-side MCP specs from the task principal's grants.

        Phase 3 单一真相源：读 principal 的 service grants（mcp_grants 的
        ``grant_key='service'`` 行），每个授权服务按 kind 走
        ``resource_reference_service._mcp_service_wire_spec``——非 stdio（builtin/
        custom/sse/node_hosted）产出网关代理 url + task identity token，上游
        密钥不下发；stdio 保留 command/args/env_template 由节点本地 spawn。
        param grants（cdp_client_id 等实例绑定）不在此展开：网关在请求时按
        token → principal → param grant 收窄实例范围。

        task identity token 每次派发现签（display_token=False，长时效语义）；
        吊销靠 principal 禁用（_disable_task_mcp_principal / _enable_task_mcp_principal
        生命周期闭环），不吊单个 token。Best-effort：单条服务推导失败只丢该条，
        不影响其余 MCP 与派发本身。
        """
        principal_id = getattr(task, "mcp_user_id", None)
        if not principal_id:
        ...
        try:
            ...
            logger.exception(
                "[user-platform] task {} principal service grants read failed", task.id
            )
            return []
        if not service_ids:
            return []
        from . import config
        from .resource_reference_service import _mcp_service_wire_spec, mcp_service_is_stdio

        # 节点自拼形态：默认（gateway_public_url 未显式配置非回环值）spec url
        # 只发相对路径，节点用服务端 hello 通告的网关 origin 拼绝对地址——远程
        # 节点不再拿到永远连不上的 127.0.0.1。管理员显式配置了对外网关地址的
        # 部署保持绝对 URL 旧形态（他们明说了网关在哪）。
        base = ""
        explicit_origin = config._explicit_gateway_origin()
        if explicit_origin:
            from urllib.parse import urlparse

            if not config._is_loopback_host(urlparse(explicit_origin).hostname):
                base = explicit_origin
        specs: list[dict] = []
        ...
        for service_id in service_ids:
            try:
                ...
                logger.exception(
                    "[user-platform] task {} granted service {} read failed",
                    task.id, service_id,
                )
                continue
            if not service or not service.get("enabled"):
            ...
            if mcp_service_is_stdio(service):
                specs.append(_mcp_service_wire_spec(service))
                continue
            if identity_token is None:
                try:
                    ...
                    logger.exception(
                        "[user-platform] task {} identity token issuance failed", task.id
                    )
                    identity_token = ""
            if not identity_token:
                # 不能静默吞掉——这是「工具选了但任务没挂上」的最常见根因
                # （token 签发失败）。留 warning 让排障能从日志直接定位，
                # 而不是对着空 mcps 猜。
                logger.warning(
                    "[user-platform] task {} granted service {} skipped: identity token issuance failed",
                    task.id, service_id,
                )
                continue
            specs.append(_mcp_service_wire_spec(
                service, identity_token=identity_token, gateway_base_url=base,
            ))
        return specs

    async def _sync_task_mcp_service_grants(self, task: Task, mcp_payload: object) -> None:
        """Translate an mcp_config request payload into principal grants.

        接受两种形态（create/update 两条路都会遇到）：routes_task 归一后的
        服务绑定（``{service_id}`` 或 ``{service_id, param_key, param_values}``——
        创建弹框「选工具后勾选实例」携带的实例绑定）和内部调用方直传的已解析
        spec（带 name）。service 行逐条写入；带 param_key 的条目把 param_values
        写成 param 行（多值=多实例，缺省=不收窄默认全量）。

        用 set_principal_grants 全量替换：创建弹框是任务 principal 授权的唯一
        编辑入口（详情页编辑器已下线），全量替换即所见即所得。空列表合法——
        撤全部授权。Best-effort：失败记日志不阻断主流程。
        """
        if not isinstance(mcp_payload, list):
            return
        principal_id = getattr(task, "mcp_user_id", None)
        if not principal_id:
            return
        try:
            ...
            for entry in mcp_payload:
                if not isinstance(entry, dict):
                    continue
                # ① 直接绑定形态：{service_id[, param_key, param_values]}。
                sid_raw = entry.get("service_id")
                if sid_raw is not None and str(sid_raw).strip():
                    ...
                    if sid not in seen_services:
                        ...
                        grants.append({"grant_key": mcp_plugin_store.GRANT_SERVICE_KEY,
                                       "grant_value": str(sid)})
                    param_key = str(entry.get("param_key") or "").strip()
                    param_values = entry.get("param_values")
                    if param_key and isinstance(param_values, list) and param_values:
                        for value in param_values:
                            value = str(value).strip()
                            if value:
                                grants.append({"grant_key": param_key, "grant_value": value})
                    continue
                # ② 团队/用户引用形态：{resource_id} 不携带 wire spec，更不能静默丢弃。
                # 引用的 owned_entity_id 指向 mcp_services.id；先按当前用户的 team
                # 可见引用校验归属，再把它归一成同一条 service grant。这样 shared
                # 环境 MCP、团队市场 MCP、个人引用 MCP 与 service_id 工具走同一派发链。
                resource_id = str(entry.get("resource_id") or "").strip()
                if resource_id:
                    from .resource_reference_service import visible_references
                    team_id = await resolve_team_id(str(task.user_id))
                    visible = await visible_references(str(task.user_id), team_id, "mcp")
                    reference = next((row for row in visible if str(row.get("id")) == resource_id), None)
                    owned_id = str((reference or {}).get("owned_entity_id") or "").strip()
                    if not owned_id:
                        logger.warning("[user-platform] task {} MCP reference {} has no owned service", task.id, resource_id)
                        continue
                    try:
                        sid = int(owned_id)
                    except (TypeError, ValueError):
                    ...
                    if sid not in seen_services:
                        seen_services.add(sid)
                        grants.append({"grant_key": mcp_plugin_store.GRANT_SERVICE_KEY, "grant_value": str(sid)})
                    continue
                # ③ 已解析 spec 形态（带 name）：按 name 反查 service id（issue-workflow
                # 等 session overlay 不在目录里，跳过）。
                ...
                if sid not in seen_services:
                    ...
                    grants.append({"grant_key": mcp_plugin_store.GRANT_SERVICE_KEY,
                                   "grant_value": str(sid)})
            if not grants and not mcp_payload:
                # 没有绑定也没有存量要清（principal 是新建的），跳过 DB 往返。
                return
            owner = str(task.user_id)
            try:
                await mcp_plugin_store.set_principal_grants(
                    int(principal_id), grants, owner_user_id=owner,
                )
            except LookupError:
                raise
            except ValueError:
                # param 校验失败（资源不存在/无权）通常是弹框与服务端资源状态
                # 竞态。降级为只写 service 行，保证任务还能派发（默认全量语义）。
                logger.warning(
                    "[user-platform] task {} param grants rejected, falling back to service-only",
                    task.id,
                )
                service_only = [g for g in grants if g["grant_key"] == mcp_plugin_store.GRANT_SERVICE_KEY]
                await mcp_plugin_store.set_principal_grants(
                    int(principal_id), service_only, owner_user_id=owner,
                )
        except Exception:
            logger.exception(
                "[user-platform] task {} mcp payload translation failed", task.id
            )

    async def _task_mcp_service_ids(self, task: Task) -> list[int]:
        """Legacy: derive authorized service ids from the retired mcp_config column.

        mcp_config 列已退役（Phase 3 不再写，Phase 4 删除）；保留本入口仅为
        迁移期诊断与既有契约测试。新代码一律走
        ``_mcp_service_ids_from_specs``（请求负载）或 grants（已落库授权）。
        """
        return await self._mcp_service_ids_from_specs(
            getattr(task, "mcp_config", None) or []
        )

    @staticmethod
    async def _mcp_service_ids_from_specs(specs: object) -> list[int]:
        """Resolve MCP config entries (specs or raw bindings) into service ids.

        Entries carrying an explicit ``service_id`` (the direct mcp_services
        binding the create-task dialog sends for builtin/admin/personal MCPs)
        map straight through; resolved specs map by the spec ``name``. Names
        that are not catalog services (issue-workflow and other per-task
        overlays, local session servers) are skipped — they ride the session
        directly, no authorization needed.
        """
        import mcp_plugin_store
        ...
        for spec in list(specs or []):
            ...
            if not service:
                ...
                # services; they ride the session directly, no authorization
                # needed.
                continue
            _collect(service["id"])
        return service_ids
    ...
    async def _disable_task_mcp_principal(self, task: Task) -> None:
        ...
        try:
            ...
            logger.exception(
                "[user-platform] task {} principal disable failed", task.id
            )
    ...
    async def _enable_task_mcp_principal(self, task: Task) -> None:
        ...
        try:
            ...
            logger.exception(
                "[user-platform] task {} principal enable failed", task.id
            )
    ...
    async def _delete_task_mcp_principal(self, task: Task) -> None:
        ...
        try:
            ...
            logger.exception(
                "[user-platform] task {} principal delete failed", task.id
            )
        task.mcp_user_id = None
    ...
    async def _park_task_api_key(self, task: Task) -> None:
        ...
        try:
            ...
            logger.exception(
                "[user-platform] task {} api key park failed", task.id
            )
    ...
    async def _resume_task_api_key(self, task: Task) -> None:
        ...
        try:
            ...
            logger.exception(
                "[user-platform] task {} api key resume failed", task.id
            )
    ...
    async def update_task(
        ...
        Resource configs (``skill_config``/``plugin_config``) are persisted as
        the authoritative base before any resync attempt, so a runtime that is
        offline, stopping, or restarting still picks up the new config on its
        next dispatch/restart. ``mcp_config`` no longer persists — it is
        translated into principal service grants (the single source of truth);
        a live runtime receives the newly granted set through
        ``apply_node_session_mcps`` (specs regenerated from grants, merged with
        the task's per-task overlay so issue-workflow survives) without a
        restart.
        """
        ...
        # Resource configs are persisted as the authoritative base before any
        # resync attempt, so a runtime that is offline, stopping, or restarting
        # still picks up the new config on its next dispatch/restart. A live
        # runtime receives the new config through ``apply_node_session_*``
        # without a restart. mcp_config 不在此列：该列退役，任务 MCP 的真相源
        # 是 principal grants，由下方 grant 写入段处理。
        resource_keys = ("mcp_config", "skill_config", "plugin_config")
        for key in ("skill_config", "plugin_config"):
            if key in fields:
                value = fields[key]
                setattr(
                    task, key,
        ...
        # MCP config changed: translate the request payload into principal
        # service grants (the retired mcp_config column is never written).
        # Empty list is a legal clear-all. The translation accepts both the
        # resolved specs (routes_task runs the resolver before update) and raw
        # {service_id} bindings from internal callers.
        if "mcp_config" in fields:
            # No principal yet (legacy row created storage-only before the
            # create-time principal existed): mint one so grants have a home.
            await self._ensure_task_mcp_principal(task)
            await self._sync_task_mcp_service_grants(task, fields.get("mcp_config"))
            ...
            if not client.enabled:
                ...
                try:
                    if "mcp_config" in config_changed:
                        # 与派发同源：从 grants 现生成 kind-aware spec（网关 url +
                        # task identity token / stdio command 行），merge per-task
                        # overlay（issue-workflow 等）后热下发。
                        await client.apply_node_session_mcps(
                            node_session_id,
                            _merge_mcp_config(
                                await self._principal_mcp_specs(task), overlay,
                            ),
                        )
    ...
    async def _stop_runtime(self, task: Task) -> None:
        ...
        try:
            ...
            logger.exception("[user-platform] task {} runtime stop failed", task.id)
            raise
        task.node_session_id = None
        ...
        if session_id:
            ...
            if client.enabled:
                try:
                    ...
                    logger.exception(
                        "[user-platform] task {} session delete failed", task_id
                    )
    ...
    async def stop_task(self, user_id: str, task_id: str, *, role: str | None = None) -> bool:
        ...
        await self._disable_task_mcp_principal(task)
        # Fire the task.paused notification (user default in the mobile notify
        # catalogue). Backgrounded: a hanging webhook must not delay the stop.
        from .notify_core import emit_notification_background
        ...
        emit_notification_background(
            ...
            kind="task",
            source="task_service",
            message=f"任务「{task.title or (task.content or '')[:40]}」已暂停",
        )
        return True
    ...
    async def _clear_stale_node_session(task: Task, session_id: str, reason: str) -> None:
        ...
        """
        logger.info(
            "[user-platform] task {} node session {} is stale ({}); clearing handle",
    ...
    async def _deliver_turn(
        ...
        if title_candidate and not str(getattr(task, "title", "") or "").strip():
            ...
            try:
                ...
                logger.warning("[user-platform] failed to backfill title for task {}", task.id)
    ...
    async def switch_task_model(
        ...
        """Switch the active model (the one handed to the runtime next turn).

        切换只换「下一轮下发给运行时的是哪个模型」，不改任务能用哪些模型——
        下拉列表的口径是父 Key 目录，由 /chat/models 提供，与本函数无关。目标
        模型按父 Key（创建时的归属）校验：合法就直接切到头部，不必预先加入任
        何集合；原快照成员保留在后（顺序不变），只是不再拿快照当切换边界——
        曾要求目标先经「添加模型」进集合，切一次就把 UI 列表塌缩成集合本身，
        用户视角即「选过一次列表就只剩它了」。子 Key 白名单始终同步为整个快照，
        保证运行时按当前模型推理不被 Key 名单拦下。
        """
        task = await self._owned_task_or_raise(user_id, task_id, role)
        ...
        # 校验永远对父 Key（目录归属）：目录里列得出的就该切得过。子 Key 白名单
        # 只是快照的执行投影，不能反过来当边界。
        validate_key = task.parent_api_key_id or task.api_key_id
        # 只校验目标本身：带上快照成员会让历史成员（如父 Key 目录后来移除了它）
        # 连坐导致整个切换被拒——切到 X 不该被无关的 Y 是否仍合法影响。
        _, resolved = await _validate_models_for_key(
            validate_key, [model_id], active_model=model_id
        )
    ...
    async def _resolve_task_active_model(self, task: Task, req: dict) -> str:
        ...
        try:
            ...
            logger.warning("[user-platform] task {} default model resolve failed", task.id)
        return ""
            ...
            async for event in iterator:
                ...
                if kind == "result":
                    ...
                    if updated:
                        ...
                        await connections.get("user_platform").execute_query(
    ...
    async def _next_event_seq(self, task_id: uuid.UUID) -> int:
        ...
        rows = await connections.get("user_platform").execute_query_dict(
    ...
    async def _record_status_transition(
        ...
        *,
        ...
        try:
            ...
            logger.debug(
                "[user-platform] status history {} {}→{} for task {} not recorded",
    ...
    async def _persist_task_event(self, task_id: uuid.UUID, event: dict) -> None:
        ...
        try:
            ...
            logger.exception("[user-platform] task {} event persist failed", task_id)
    ...
    async def _load_frame_contents(task_id: uuid.UUID, rows: list) -> dict[str, dict]:
        ...
        try:
            ...
            logger.exception("[user-platform] task {} frame content load failed", task_id)
            return {}
        merged: dict[str, dict] = {}
    ...
    async def task_stats(self, user_id: str, task_id: str, *, role: str | None = None) -> dict:
        ...
        if task is None:
            return {}
        # Read from request_logs (the gateway's billing ledger) rather than
        # mc_task_usage_stats: that aggregate table is never written, so the
        # detail header showed 0 tokens forever. Same source the 请求日志 panel
        # already reads; also returns a per-model breakdown for the header.
        from db import PostgresClient

        return await PostgresClient.task_request_token_stats(str(task.id))
    ...
    async def list_task_request_logs(
        ...
        *,
        ...
        Ownership is resolved through the Task ledger (user_platform
    ...
    async def rotate_task_api_key(
        ...
        if node_session_id:
            try:
                ...
                logger.exception(
                    "[user-platform] task {} key rotation node-config failed; rolling back",
                    task.id,
                )
