...
async def consumer_item(module: str, item: str) -> dict:
    ...
    if await store.is_populated():
        ...
        # 与消费侧索引同口径：只放行 published。草稿/hidden 单条也不能漏给用户
        # （否则知道 item_id 就能直取未发布的 manifest，包括 agency-agents 这类
        # 同步落库即 draft 的源）。
...
async def consumer_mobile_version() -> dict:
    ...
    # 命名文件，落到正确的 .apk 文件名而不是裸 ``android``。
    android_filename = str(android.get("filename") or "").replace('"', "").replace("/", "")
    android_proxy = ""
    if android.get("download_url"):
        android_proxy = (
            f"/api/v1/marketplace/consumer/download/mobile-android/{android_filename}"
            if android_filename else "/api/v1/marketplace/consumer/download/mobile-android"
        )
    return {
        "version": str(snapshot.get("version") or ""),
        "version_notes": str(snapshot.get("version_notes") or ""),
        ...
        "android": {
            ...
            "proxy_download_url": android_proxy,
...
async def consumer_device_control_version() -> dict:
    ...
    def _platform_block(platform: str) -> dict:
        ...
        # 尾段带真实文件名：忽略 Content-Disposition 的手机浏览器按 URL 尾段
        # 命名文件，落到正确的 .apk/.ipa 文件名而不是裸 ``platform``。
        filename = str(asset.get("filename") or "").replace('"', "").replace("/", "")
        proxy = ""
        if asset.get("download_url"):
            base = f"/api/v1/marketplace/consumer/download/device-control/{platform}"
            proxy = f"{base}/{filename}" if filename else base
        return {
            "version": str(asset.get("version") or snapshot.get("version") or ""),
            "download_url": str(asset.get("download_url") or ""),
            "proxy_download_url": proxy,
...
@router.get("/consumer/download/mobile-android/{filename}")
async def consumer_download_mobile_android(filename: str = "") -> StreamingResponse:
    """移动控制端（Android）APK 代理下载：与 ``/consumer/mobile-version`` 同源公开。

    App 内「检查更新」与「关于 → 下载最新安装包」都走这里；手机不需要能直连 GitHub。
    尾段 ``filename`` 仅装饰：忽略 Content-Disposition 的手机浏览器按 URL 尾段
    命名文件，带真实 APK 文件名的链接存下来就是正确文件名。
    """
    return await _proxy_download_mobile_android_apk()


@router.get("/consumer/download/device-control/{platform}")
@router.get("/consumer/download/device-control/{platform}/{filename}")
async def consumer_download_device_control(platform: str, filename: str = "") -> StreamingResponse:
    ...
    尾段 ``filename`` 仅装饰：部分手机浏览器忽略 Content-Disposition、按 URL
    尾段命名文件，带真实 APK/IPA 文件名的链接存下来就是正确文件名；不带的
    旧客户端走无尾段路径，行为不变。
    """
...
# ── 社区运营配置（技术交流群 + 社区通知）────────────────────────────────────
# 与市场仓库无关：公开读挂 router 常驻可用（admin_router 会被 _require_market_writable
# 在未配 token 时 404，社区配置不该吃这个门禁）；写路径复用平台管理员登录。


@router.get("/community-config")
async def get_community_config_public() -> dict:
    """用户控制台「技术交流群 / 社区信息」弹窗的数据源，无鉴权（对齐 /status）。"""
    return community_public_view(await get_community_config_async())


@router.get("/admin/community-config")
async def get_community_config_route(_: User = Depends(_require_admin)) -> dict:
    """管理端读取社区运营配置（与公开视图同形）。"""
    return community_public_view(await get_community_config_async())


@router.put("/admin/community-config")
async def update_community_config_route(
    body: dict = Body(...), _: User = Depends(_require_admin)
) -> dict:
    """保存社区运营配置：``groups`` / ``notice`` 整段替换，未传的不动。"""
    if not isinstance(body, dict):
        raise HTTPException(status_code=400, detail="配置必须是对象")
    config = await update_community_config(body)
...
async def upload_node_version(
    ...
    for upload in files:
        ...
        try:
            ...
            # 写盘失败（含 errno 28 磁盘满）必须清掉半截文件，否则留在 temp 里把盘吃满。
            upload_jobs.cleanup_job(job_id)
            if exc.errno == 28:
                raise HTTPException(status_code=507, detail=f"磁盘空间不足，无法暂存上传文件：{filename}") from exc
...
async def sync_agency_agents_route(
    ...
    """手动同步 agency-agents（后台异步执行，立即返回）。

    前端轮询 ``GET /admin/marketplace/agency-agents/last-sync`` 看实时进度+日志。
    ref 优先级：body > 配置 ``agency_agents_ref`` > 内置基线 commit。
    """
    from . import agency_agents_convert
    from .source_config import get_source_config_async

    if agency_agents_convert._progress.get("running"):
        raise HTTPException(status_code=409, detail="已有同步在运行中，请等当前任务完成")
    cfg = await get_source_config_async()
    ref = (str(body.get("ref") or "").strip() or str(cfg.get("agency_agents_ref") or "").strip()
...
async def agency_agents_last_sync_route(_: User = Depends(_require_admin)) -> dict:
    ...
    内存态优先（有实时进度）；进程刚重启内存为「从未同步」时回落 DB 执行记录
    （marketplace_sync_runs 最新一行），保证面板重启后仍显示最近一次的日志。
    """
    from . import agency_agents_convert
    memory = agency_agents_convert.last_result()
    if memory.get("ran_at"):
        return memory
    return await _sync_run_fallback("agency-agents", memory)


async def _sync_run_fallback(source: str, memory: dict) -> dict:
    """DB 执行记录 → last-sync 端点形态。进程刚重启、内存为「从未同步」时用它。"""
    import marketplace_sync_run_store

    row = await marketplace_sync_run_store.latest_run(source)
    if not row:
        return memory
    return {
        "ran_at": row.get("finished_at") or row.get("started_at") or "",
        "ok": row.get("ok"),
        "detail": row.get("detail") or "",
        "logs": row.get("logs") or [],
        "run_by": row.get("run_by") or "",
...
async def sync_agency_agents_zh_route(
    ...
    """手动同步 agency-agents-zh（后台异步执行，立即返回）。

    ref 优先级：body > 配置 ``agency_agents_zh_ref`` > 中文源 main 分支。
    （fork 与英文源历史分叉，英文基线 commit 在 fork 里不存在——之前用英文
    DEFAULT_REF 兜底导致每次 404。）
    """
    from . import agency_agents_convert
    from .source_config import get_source_config_async
    ...
    if agency_agents_convert._progress.get("running"):
        raise HTTPException(status_code=409, detail="已有同步在运行中，请等当前任务完成")
    cfg = await get_source_config_async()
    ref = (str(body.get("ref") or "").strip() or str(cfg.get("agency_agents_zh_ref") or "").strip()
...
async def agency_agents_zh_last_sync_route(_: User = Depends(_require_admin)) -> dict:
    """上次同步结果（中文源）+ 实时进度。内存为空回落 DB 执行记录（同英文源口径）。"""
    from . import agency_agents_convert
    memory = agency_agents_convert.last_result_zh()
    if memory.get("ran_at"):
        return memory
    return await _sync_run_fallback("agency-agents-zh", memory)
...
async def agentscope_last_sync_route(_: User = Depends(_require_admin)) -> dict:
    ...
    from . import agentscope_convert
    memory = agentscope_convert.last_result()
    if memory.get("ran_at"):
        return memory
...
async def leaderboard_status(_: User = Depends(_require_admin)) -> dict:
    """同步开关与上次结果（含实时进度）。未启用时前端据此显示引导而不是空列表。

    ``last_sync``：内存态（带 progress）；进程刚重启内存为空时回落 DB 执行记录，
    保证面板重启后仍能看到最近一次的日志与时间。
    """
    ...
    if not last_sync.get("ran_at"):
        last_sync = await _sync_run_fallback("agent-leaderboard", last_sync)
    return {
...
async def update_leaderboard_item(
    ...
    """管理员收口归类/可安装性/标签/启动方式/名次。**不含发布**——发布是独立的
    推送动作（POST /publish 入队，worker 校验门禁后翻状态）。patch 里的 ``status``
    在 store 层被忽略：编辑保存绝不静默发布。"""
    import marketplace_leaderboard_store as store
...
@admin_router.post("/admin/leaderboard/items/{item_id}/reprobe")
async def reprobe_leaderboard_item(
    ...
    recognized = await github_recognize.recognize_repo(repo_full_name, force_type=force_type)
    if recognized.get("error"):
        raise HTTPException(status_code=422, detail=recognized["error"])

    install_spec = recognized.get("install_spec") or {}
    launch_spec = recognized.get("launch_spec") or {}
    skill_entries = recognized.get("skill_entries") or (install_spec.get("skill") or {}).get("entries") or []
    ...
    try:
        updated = await store.update_curation(item_id, patch)
    except ValueError as exc:
        raise HTTPException(status_code=400, detail=str(exc)) from exc
    if updated is None:
        raise HTTPException(status_code=404, detail="条目不存在")
...
@admin_router.post("/admin/leaderboard/publish", status_code=202)
async def publish_leaderboard_items(
    body: dict = Body(...), user: User = Depends(_require_admin)
) -> dict:
    """批量推送发布（也用于单条：传一个 id）。**入队即返回**，后台 worker 逐条校验。

    校验语义与旧同步发布一致（publish_items：已发布幂等 skipped、可装无分类先从
    上游推导、MCP launch_spec 门禁软/硬由 ``leaderboard_require_verified`` 控制），
    但执行移到 publisher worker——门禁不过的条目进 failed 带原因，指数退避自动
    重试，达上限置 failed 供管理页手动重试（发布队列面板 module=leaderboard 可见）。
    """
    import marketplace_leaderboard_store as store
    ...
    if not enqueued:
        # 全部已发布/不存在：无 job 可跑，直接把幂等结果回给前端（仍是 202 语义）。
        return {"queued": [], "skipped": skipped, "requested": len(ids), "detail": "所选条目均已是发布态，无需推送"}
    return {
        "queued": enqueued,
        "skipped": skipped,
        "requested": len(ids),
        "detail": f"已提交推送 {len(enqueued)} 项，发布队列处理中（校验不过的条目会带原因进失败列表）",
    }
...
@admin_router.post("/admin/leaderboard/delete")
async def delete_leaderboard_items(
    body: dict = Body(...), _: User = Depends(_require_admin)
) -> dict:
    """批量硬删除候选池条目。**破坏性**——行直接删掉（含已发布条目，用户侧立即
    不可见）；上游榜单再次同步会把同名仓库重新拉回来当草稿。"""
    import marketplace_leaderboard_store as store
    ...
    if not ids:
        raise HTTPException(status_code=400, detail="请至少选择一个条目")
    removed = await store.delete_items(ids)
