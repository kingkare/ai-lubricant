...
# Repository-listing pagination bounds (parity with upstream).
_DEFAULT_REPO_PAGE_SIZE = 20
_MAX_REPO_PAGE_SIZE = 100
# Repository result cache TTL: 7 days (matches upstream's go-cache expiry).
...
class _RepoCache:
    ...
    Mirrors upstream's in-process go-cache: full lists are keyed by
...
class GitService:
    ...
    async def get_identity(
        ...
        *,
        ...
        with upstream). Upstream failures are logged (without secrets) and the
    ...
    async def _fetch_repositories(
        ...
        *,
        ...
        try:
            ...
            logger.warning(
                "[user-platform] list repositories failed: platform={} identity_id={} error={}",
                platform,
                identity.id,
                exc,
            )
    ...
    def _prefetch_repositories(self, identity: GitIdentity) -> None:
        ...
        Mirrors upstream's post-add/update prefetch: never blocks CRUD, never
        ...
        async def _run() -> None:
            try:
                ...
                logger.debug(
                    "[user-platform] repo prefetch skipped: identity_id={}", identity.id
                )
    ...
    async def list_branches(
        ...
        try:
            ...
            logger.warning(
                "[user-platform] list branches failed: platform={} identity_id={} error={}",
                platform,
                identity.id,
                exc,
            )
