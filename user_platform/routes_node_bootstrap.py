...
router = APIRouter(prefix="/api/v1/public/nodes", tags=["user-platform-nodes-public"])
...
async def install_script(node_id: str, request: Request, method: str | None = None):
    ...
    # 入驻排障主入口：一行看清烘给节点的拨号地址/代理/角色。节点连不上时先看这行——
    # server_url 是回环（127.0.0.1）说明远程节点会拨到它自己；proxy 为空说明 GitHub
    # 直连下载（部分网络会挂死在 downloading）。
    from loguru import logger as _logger

    _logger.info(
        "[node-install] install.sh rendered: node_id={} role={} method={} server_url={} proxy_mode={} proxy_url={}",
        node_id,
        str(bootstrap.get("role") or ""),
        str(bootstrap.get("startup_method") or ""),
        str(bootstrap.get("server_url") or ""),
        str(proxy_fields.get("proxy_mode") or "direct"),
        str(proxy_fields.get("proxy_url") or proxy_fields.get("proxy_url_prefix") or ""),
    )
...
# macOS 没有 sha256sum（自带 shasum -a 256）；两个都试，都不行才报校验失败。
[ -z "$sha" ] || echo "$sha  $bin.tmp" | sha256sum -c - >/dev/null 2>&1 || echo "$sha  $bin.tmp" | shasum -a 256 -c - >/dev/null 2>&1 || {{ echo "checksum mismatch"; rm -f "$bin.tmp"; exit 1; }}
mv "$bin.tmp" "$bin"
chmod +x "$bin"
